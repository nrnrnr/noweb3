...
A chunk may optionally end with a return statement (\see{return}).
...
Moreover, tables must be explicitly created before being used
...
\produc{sc}{\opt{\ter{;}}}
\end{Produc}%
A \IndexVerb{return} statement can only be written
as the last statement of a block.
This restriction also avoids some ``statement not reached'' errors.
This rule is enforced syntactically, in the Lua parser.
...
find a match in \emph{case\_arm}.
...
\rwd{else} plus \darrow, or if there is no such block, the interpreter does
nothing.
...
e.g., \verb'A-Z' specifies the uppercase characters.
...
called when Lua tries to assign an index to a non-table value.
...
\end{verbatim}
Both functions return a result code:
...
for instance,
the comparison of two \verb"lua_Object's" is of no significance.
...
\end{verbatim}
\verb'lua_getnumber' converts a \verb'lua_Object' to a double-precision floating
point number.
...
a pointer will be valid after the block ends.
...
\item[{\tt \%L}] --- represents all non-lowercase letter characters.
\item[{\tt \%s}] --- represents all space characters.
\item[{\tt \%S}] --- represents all non-space characters.
\item[{\tt \%u}] --- represents all uppercase letters.
\item[{\tt \%U}] --- represents all non-uppercase letter characters.
...
A pattern item may also have the form \verb'%n',
...
It returns a result code, which is implementation-defined.
...
A hook is disabled when its value is \verb'NULL' (0),
